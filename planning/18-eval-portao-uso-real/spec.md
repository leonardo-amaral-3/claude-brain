...
**Emenda 2026-09-14 (task 6): a instalação nem sempre está fora do git, e quando está dentro, a guarda de branch lê o git e não o marcador.** A spec supõe que o eval roda sempre numa instalação que não é repositório. Isso vale para esta máquina (`~/notoria/claude/brain-mcp`), mas não para o caminho padrão do repo público: `install.sh:22` e `install.ps1:33` registram o MCP em `<repo>/brain-mcp/dist/index.js`, e aí a instalação é o checkout. Também não vale para a verificação deste card, que desde a task 1 roda da worktree com junction de `node_modules`. Nos dois casos o marcador ou não existe ou descreve outra pasta (aqui diz `branch=main`). A guarda recusaria para sempre, e a única saída seria `sync push` da branch com a ordenação invertida, entregando busca invertida a toda sessão da máquina durante a prova do CA3. Portanto:

1. **A pasta onde o eval roda está dentro de uma worktree git deste repo** (a raiz dela contém esta `brain-mcp/`) → o `dist/` foi compilado dela, e a branch do head é a branch dessa worktree. O marcador não é lido para a branch. A ordem de resolução do checkout passa a ser `--repo` → `BRAIN_REPO` → a própria worktree → `worktree=` do marcador.
2. **A pasta não é git** → vale o texto original: a branch do head é o `branch=` do marcador. Se o marcador não existe e o checkout veio de `--repo`/`BRAIN_REPO`, o resultado é **código `2`**. Sem saber de que branch o `dist/` veio, a guarda não tem o que comparar, e passar em silêncio seria o "não caiu com toda a confiança" da decisão técnica 10. A decisão 9 chamava o `--repo` de escape para quem não tem marcador; pelo caminho padrão, quem não tem marcador é quem instalou do clone, e esse cai no item 1 sem precisar de flag.
3. **A guarda de frescor não muda.** No item 1 é ela que pega um `git checkout` para outra branch sem rebuild: o checkout reescreve os `src/*.ts` que diferem, e o `dist/` fica mais velho que eles.
